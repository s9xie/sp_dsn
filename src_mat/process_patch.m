...
parfor i = 1:size(train_files,1)
    ...
    for id = 2:size(map,1) % 1 is always blank
        ...
        for np = 1: numPatches,
            ...
            patchName = [patchFolderName, train_files{i}(1:end-4), '_', labelMap(label_string), '_' ,int2str(np), '.bmp'];
            contextName = [contextFolderName, train_files{i}(1:end-4), '_', labelMap(label_string), '_' ,int2str(np), '.bmp'];
